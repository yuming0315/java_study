//package chat;
//
//import java.nio.charset.StandardCharsets;
//import java.util.Arrays;
//import java.util.Base64;
//
//public class Parser {
//	// protocol에서는 암호풀린 문자열가지고 처리
//	private String protocol(String msg) {
//		String request = null;
//		String[] tokens = msg.split(":");
//		switch (tokens.length) {
//		case 1:
//			request = length1(msg);
//			break;
//
//
//			
//			
//			
//		case 3:
//			request = length3(msg);
//			break;
//		}
//		return request;
//	}
//
//	private String length1(String msg) {
//		String request = null;
//		String[] tokens = msg.split(" ");
//		switch (tokens.length) {
//		case 2:
//			if ("RENAME".equals(tokens[0])||"JOIN".equals(tokens[0])) {
//				request = length2(msg);
//			}
//			else {
//				request = makeMsg(msg);
//			}
//			break;
//		default:
//			if ("QUIT".equals(tokens[0])) {
//				request = tokens[0];
//			} 
//			else { // snd msg 일 경우
//				request = makeMsg(msg);
//			}
//		}
//		return request;
//	}
//
//	private String makeMsg(String msg) {
//		return new String(encoding(msg));
//		//new String(encoding(msg),StandardCharsets.UTF_8);
//	}
//	
//	private String length2(String str) {
//		
//		
//		return str.replace(" ", ":");
//	}
//
//	private String length3(String data) {
//		String request = null;
//		
//		if(data == null ) {
//			log("대화방과 연결이 끊겼습니다.");
//			break;
//		}
//		else if("QUIT:OK".equals(data)) {
//			log("접속이 종료되었습니다.");
//			break;
//		}
//		else if("ERROR".equals(data)) {
//			log("메시지 전달 오류");
//			continue;
//		}
//		
//		String[] tokens = data.split(":");
//		
//		switch(tokens[0]) {
//		case "SND":
//			print(tokens[tokens.length-1]+":"+addString(Arrays.copyOfRange(tokens,1,tokens.length-1)));
//			break;
//			
//		case "RENAME":
//			if(isMsg(tokens)) {
//				reName(tokens);
//			}
//			else {
//				reNameMe(tokens);
//			}
//			break;
//		
//		case "JOIN":
//			if(isMsg(tokens)) {
//				join(tokens);
//			}
//			else {
//				joinMe(tokens);
//			}
//			break;
//		case "QUIT":
//			Quit(tokens[1]);
//			break;
//		}
//		
//		
//	}
//	private String addString(String[] str) {
//		String text="";
//		for(String s : str) {
//			text+=s;
//		}
//		return text;
//	}
//	private void reNameMe(String[] str) {
//		print(str[2]+"로 별명이 변경되었습니다.");
//	}
//	private void Quit(String str) {
//		print(str+"님이 퇴장하셨습니다.");
//	}
//	
//	private void joinMe(String[] str) {
//		print(str[1]+"님 환영합니다.");
//	}
//
//	private boolean isMsg(String[] str) {
//		return !str[str.length-1].equals("OK");
//	}
//	
//	
//	private void join(String...str ) {
//		print(str[1]+"님이 입장하셨습니다.");
//	}
//	
//	private void reName(String...str ) {
//		print(str[2]+"님이 "+str[1]+"으로 별명을 변경하였습니다");
//	}
//	
//	
//	
//	public byte[] msg(String str) {
//		return encoding(protocol(str));
//	}
//	
//	public byte[] msg(String str,String str2) {
//		return msg(str+" "+str2);
//	}
//
//	public String msg(byte[] str) {
//		return protocol(decoding(str));
//	}
//
//	private byte[] encoding(String str) {
//		return Base64.getEncoder().encode(str.getBytes(StandardCharsets.UTF_8));
//	}
//
//	private String decoding(byte[] str) {
//		return new String(Base64.getDecoder().decode(str), StandardCharsets.UTF_8);
//	}
//
//}
